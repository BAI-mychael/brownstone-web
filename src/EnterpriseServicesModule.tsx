...
`.trim();
    ...
    try {
      const { error } = await supabase
        .from('leads')
        .insert([{
          email: auditAnswers.email,
          full_name: 'NIST Audit Respondent',
          service_interest: 'ai_audit',
          source: 'nist_risk_calculator',
          message: telemetryBrief
        }]);

      if (error) throw error;
      setAuditSubmitted(true);
    } catch (error: any) {
      console.error('NIST audit save failed:', error);
      setAuditError(error.message || 'Transmission failed. Please try again.');
    } finally {
  ...
  const executeAutonomousTriage = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    // Log simulated operational triage diagnostic to Supabase DB leads table
    const incidentBrief = `
...
`.trim();
    ...
    try {
      await supabase
        .from('leads')
        .insert([{
          email: triagePayload.contact || 'ops-admin@brownstone.ai',
          full_name: `Triage Incident System (${trackingId})`,
          service_interest: 'msp',
          source: 'autonomous_triage_simulator',
          message: incidentBrief
        }]);
    } catch (dbError) {
      console.error('Failed to log simulated triage incident:', dbError);
    }
